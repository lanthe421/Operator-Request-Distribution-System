# 🎯 Operator Request Distribution System

Мини-CRM система для автоматического распределения входящих обращений от пользователей между операторами с учетом их загрузки и компетенций.

## ✨ Особенности

- 🤖 **Автоматическое распределение** - обращения автоматически назначаются операторам на основе весовых коэффициентов
- ⚖️ **Вероятностное распределение** - операторы с большим весом получают больше обращений
- 📊 **Контроль загрузки** - учет текущей загрузки и максимального лимита каждого оператора
- 🔄 **Гибкая настройка** - разные веса для разных источников обращений
- 📈 **Статистика в реальном времени** - мониторинг загрузки и распределения
- 🧪 **Property-Based Testing** - тестирование с использованием Hypothesis для гарантии корректности

## 🚀 Быстрый старт

### Установка

```bash
# Клонировать репозиторий
git clone <repository-url>
cd operator-request-distribution

# Установить зависимости
pip install -r requirements.txt

# Применить миграции базы данных
alembic upgrade head

# Запустить сервер
python main.py
```

Сервер будет доступен по адресу: **http://localhost:8000**

### Интерактивная документация

Откройте в браузере: **http://localhost:8000/docs**

## 📖 Использование

### 1. Создайте операторов

```python
import requests

response = requests.post(
    "http://localhost:8000/api/v1/operators",
    json={
        "name": "Иван Петров",
        "max_load_limit": 10
    }
)
```

### 2. Создайте источники обращений

```python
response = requests.post(
    "http://localhost:8000/api/v1/sources",
    json={
        "name": "Telegram Bot",
        "identifier": "telegram"
    }
)
```

### 3. Настройте веса операторов

```python
response = requests.post(
    "http://localhost:8000/api/v1/sources/1/operators",
    json={
        "weights": [
            {"operator_id": 1, "weight": 50},
            {"operator_id": 2, "weight": 30},
            {"operator_id": 3, "weight": 20}
        ]
    }
)
```

### 4. Создайте обращение

```python
response = requests.post(
    "http://localhost:8000/api/v1/requests",
    json={
        "user_identifier": "user@example.com",
        "source_id": 1,
        "message": "Здравствуйте! Нужна помощь"
    }
)
# Система автоматически назначит оператора!
```

### 5. Просмотрите статистику

```python
# Загрузка операторов
response = requests.get("http://localhost:8000/api/v1/stats/operators-load")

# Распределение обращений
response = requests.get("http://localhost:8000/api/v1/stats/requests-distribution")
```

## 🏗️ Архитектура

Система построена на трехслойной архитектуре:

```
┌─────────────────────────────────────┐
│         API Layer (FastAPI)         │
│  - Request validation (Pydantic)    │
│  - Response serialization           │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│        Service Layer                │
│  - Business logic                   │
│  - Distribution algorithm           │
│  - Transaction management           │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│      Repository Layer               │
│  - Data access (SQLAlchemy)         │
│  - Query optimization               │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│      Database (SQLite)              │
└─────────────────────────────────────┘
```

## 📊 Алгоритм распределения

Система использует метод **накопительных весов** (cumulative weights) для вероятностного выбора:

1. Выбираются доступные операторы (`is_active=True`, `current_load < max_load_limit`, есть вес для источника)
2. Вычисляется сумма всех весов
3. Генерируется случайное число от 0 до суммы весов
4. Выбирается оператор, чей диапазон содержит это число

**Пример:**
- Оператор A: вес 50 → вероятность 50%
- Оператор B: вес 30 → вероятность 30%
- Оператор C: вес 20 → вероятность 20%

## 🧪 Тестирование

Проект включает комплексное тестирование:

### Запуск всех тестов

```bash
pytest -v
```

### Типы тестов

- **Unit Tests** - тестирование отдельных компонентов
- **Integration Tests** - тестирование API endpoints
- **Property-Based Tests** - тестирование с Hypothesis (100+ итераций на свойство)

### Property-Based Testing

Система использует [Hypothesis](https://hypothesis.readthedocs.io/) для проверки 30 корректных свойств:

- Инициализация операторов
- Валидация данных
- Распределение обращений
- Статистические свойства
- Целостность данных

```bash
# Запустить только property-based тесты
pytest tests/property/ -v
```

## 📁 Структура проекта

```
.
├── app/
│   ├── api/v1/          # API endpoints
│   ├── models/          # SQLAlchemy модели
│   ├── schemas/         # Pydantic схемы
│   ├── services/        # Бизнес-логика
│   ├── repositories/    # Доступ к данным
│   ├── core/            # Конфигурация
│   └── utils/           # Утилиты
├── tests/
│   ├── unit/            # Unit тесты
│   ├── integration/     # Integration тесты
│   └── property/        # Property-based тесты
├── alembic/             # Миграции БД
├── .kiro/specs/         # Спецификации
├── main.py              # Точка входа
├── requirements.txt     # Зависимости
└── README.md
```

## 🛠️ Технологии

- **FastAPI** - современный веб-фреймворк
- **SQLAlchemy** - ORM для работы с БД
- **Alembic** - миграции базы данных
- **Pydantic** - валидация данных
- **Hypothesis** - property-based testing
- **pytest** - тестирование
- **SQLite** - база данных (легко заменить на PostgreSQL)

## 📚 API Endpoints

### Операторы
- `POST /api/v1/operators` - создать оператора
- `GET /api/v1/operators` - список операторов
- `PUT /api/v1/operators/{id}` - обновить оператора
- `PUT /api/v1/operators/{id}/toggle-active` - активировать/деактивировать

### Источники
- `POST /api/v1/sources` - создать источник
- `GET /api/v1/sources` - список источников
- `POST /api/v1/sources/{id}/operators` - настроить веса

### Обращения
- `POST /api/v1/requests` - создать обращение (автоматическое распределение)
- `GET /api/v1/requests` - список обращений
- `GET /api/v1/requests/{id}` - детали обращения

### Статистика
- `GET /api/v1/stats/operators-load` - загрузка операторов
- `GET /api/v1/stats/requests-distribution` - распределение обращений

## 🔧 Конфигурация

Создайте файл `.env`:

```env
DATABASE_URL=sqlite:///./crm.db
API_V1_PREFIX=/api/v1
DEBUG=False
HOST=0.0.0.0
PORT=8000
```

## 📈 Производительность

Система включает оптимизированные индексы для быстрых запросов:

- Композитный индекс на `operators(is_active, current_load)`
- Индекс на `operator_source_weights(source_id)`
- Композитный индекс на `requests(operator_id, source_id, status)`
- Индекс на `users(identifier)`

## 🤝 Разработка

### Создание миграции

```bash
alembic revision --autogenerate -m "Description"
alembic upgrade head
```

### Запуск в режиме разработки

```bash
uvicorn main:app --reload
```

### Демо-скрипты

```bash
# Показать статус системы
python show_status.py

# Быстрая демонстрация
python quick_demo.py

# Полная демонстрация
python demo_final.py

# Настройка демо-данных
python setup_demo.py
```

## 📝 Документация

- **START_HERE.md** - быстрый старт
- **USAGE_GUIDE_RU.md** - подробное руководство
- **DEMO_COMPLETE.md** - примеры использования
- **.kiro/specs/** - полная спецификация проекта

## 🎯 Примеры использования

### Массовое создание обращений

```python
for i in range(10):
    requests.post(
        "http://localhost:8000/api/v1/requests",
        json={
            "user_identifier": f"user{i}@example.com",
            "source_id": 1,
            "message": f"Обращение #{i}"
        }
    )
```

### Мониторинг перегрузки

```python
response = requests.get("http://localhost:8000/api/v1/stats/operators-load")
for op in response.json():
    if op['load_percentage'] > 80:
        print(f"⚠️ Оператор {op['operator_name']} перегружен!")
```

### Динамическая балансировка

```python
# Увеличить лимит перегруженного оператора
requests.put(
    "http://localhost:8000/api/v1/operators/1",
    json={"max_load_limit": 20}
)
```

## 📄 Лицензия

MIT License

## 👨‍💻 Автор

Разработано с использованием спецификаций и property-based testing для гарантии корректности.

---

**Документация API:** http://localhost:8000/docs

**Полное руководство:** [USAGE_GUIDE_RU.md](USAGE_GUIDE_RU.md)
